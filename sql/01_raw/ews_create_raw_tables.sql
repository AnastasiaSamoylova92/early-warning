...
DROP TABLE IF EXISTS raw.factMarketSignals;
DROP TABLE IF EXISTS raw.factMarketActivities;
GO
...
CREATE TABLE raw.factMarketActivities (
    year_month DATE,
    ...
    campaign_spend_eur DECIMAL(18,2),
    campaign_impressions INT,
    campaign_clicks INT,
    campaign_ctr_pct DECIMAL(9,3),
    ...
    cost_per_lead_eur DECIMAL(18,2),

    regional_crm_activity_index DECIMAL(9,2),
    pipeline_interest_index DECIMAL(9,2)
);

INSERT INTO raw.factMarketActivities
SELECT *
FROM raw.factMarketActivities_import;

SELECT TOP (10) *
FROM raw.factMarketActivities;

CREATE TABLE raw.factMarketSignals (
    year_month DATE,
    ...
    market_demand_index DECIMAL(9,2),
    market_growth_pct DECIMAL(9,2),
    competitor_pressure_index DECIMAL(9,2),
    seasonality_index DECIMAL(9,2),
    macro_business_index DECIMAL(9,2),
    supply_pressure_index DECIMAL(9,2),
    pipeline_interest_index DECIMAL(9,2),

    demand_shock_flag BIT,

    market_opportunity_score DECIMAL(9,2),
    regional_market_growth_factor DECIMAL(9,6)
);

INSERT INTO raw.factMarketSignals
SELECT *
FROM raw.factMarketSignals_import;
